...
3. Railway is detected through its documented `RAILWAY_PROJECT_ID`, `RAILWAY_ENVIRONMENT_ID`, or `RAILWAY_ENVIRONMENT_NAME` variables, so public requests use `X-Real-IP` for rate-limit identity.
4. Leave `TRUST_PROXY` unset on Railway. For another known reverse proxy, set `TRUST_PROXY=true`; for direct exposure, set `TRUST_PROXY=false`.
5. Verify that these public paths return `404`: `/server.js`, `/package.json`, `/src/server/routes/media-routes.js`, `/mobile/README.md`, and `/backups/`.
6. Verify unauthenticated requests to `/api/render`, `/api/tile-match`, `/api/image-data-url`, `/api/render-feedback`, and `/api/business-status` return `403` or `429`.
7. Verify `/api/server-control` returns `405` and cannot create `tmp/server-control/stop.flag`.
8. Verify production responses include CSP, HSTS, `X-Frame-Options`, and `Referrer-Policy` headers.
